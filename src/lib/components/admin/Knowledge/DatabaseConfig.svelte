<script lang="ts">
    ...
    import { getVectorDBConfig } from '$lib/apis/retrieval';
    ...
    export let availableTypes: VectorDBType[] = ['milvus', 'qdrant', 'opensearch', 'pgvector', 'chroma'];
    ...
    export function initializeFromResponse(response: VectorDBConfigResponse) {
        config = {
            type: response.current_db,
            config: response.config
        };
        availableTypes = response.available_dbs;
    }
    ...
    onMount(async () => {
        try {
            ...
            availableTypes = vectorConfig.available_dbs || availableTypes;
        } catch (error) {
            console.error('Failed to load vector database configuration:', error);
            toast.error(t('Failed to load vector database configuration'));
        }
    });
</script>
