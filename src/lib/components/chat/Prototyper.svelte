<script lang="ts">
	...
	import CodeChunks from './CodeChunks.svelte';

	export let chatIdProp = '';

	let artifactContentIndex = 0;
	let loaded = false;
	...
	const sendPromptOpenAI = async (model, userPrompt, responseMessageId, _chatId) => {
		...
		eventTarget.dispatchEvent(
			new CustomEvent('chat:start', {
				detail: { id: responseMessageId }
			})
		);
		await tick();

		try {
			const stream = model?.info?.params?.stream_response ?? 
							$settings?.params?.stream_response ?? 
							params?.stream_response ?? 
							true;
		...
		let lastMessageContentPart = getMessageContentParts(
			responseMessage.content,
			$config?.audio?.tts?.split_on ?? 'punctuation'
		)?.at(-1) ?? '';
...
{#if !chatIdProp || (loaded && chatIdProp)}
	...
	>
		...
		<PaneGroup direction="horizontal" class="w-full h-full">
			<Pane defaultSize={60} class="h-full flex w-full relative">
			...
			<Pane defaultSize={40} class="h-full flex flex-col">
				<Pane defaultSize={50} class="w-full">
					<Artifacts 
						{history}
						{selectedModels}
						bind:selectedContentIdx={artifactContentIndex}
					/>
				</Pane>
				...
				<Pane defaultSize={50} class="w-full">
					<CodeChunks 
						{history}
						{selectedModels}
						activeArtifactIndex={artifactContentIndex}
					/>
				</Pane>
			</Pane>
		</PaneGroup>
...
<style>
	.code-chunks-container {
		scrollbar-width: thin;
	}

	:global(.dark) .code-chunks-container {
